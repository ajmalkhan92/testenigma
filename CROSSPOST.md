...
1. Publish the post on the Ghost blog first (push to `main`, let the
   `publish.yml` workflow deploy it, confirm it's live at its URL).
...
backdates the story to match, so SEO credit stays with the Ghost blog.
